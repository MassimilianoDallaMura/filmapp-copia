...
export class AuthService {
  ...
  login(data: { email: string; password: string }) {  // riceve emil e pass in maniera volante e non permanente 
    return this.http.post<AuthData>(`${this.apiURL}login`, data).pipe( //dobbiamo rivecere qualcosa e quindi va incanalato in un flusso. Usiamo pipe
        tap((data) => {   // tap operatore di utility per ricevere un valore e mandarlo da un'altra parte. map si aspetta di dover anche fare qualcosa
  ...
  signUp(data:Register) {
    return this.http.post(`${this.apiURL}register`, data)   //  metodo che scriverà l'utente dentro il json. parte 1 del flusso di registrazione. register è l'endpoint (json documentation vuole così)
